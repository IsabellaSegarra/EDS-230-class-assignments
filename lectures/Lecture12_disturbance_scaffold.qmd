...
-   try changing the harvesting rate to see what happens
    ...
    ```{r}
    source(here("R/modified_dpopgrowth.R"))
    my_dharvest
    ...
    # watch it grow
    forest <- ode(y = postfire_initial_carbon, times = , dpopgrowth, parms = forestparms)
...
# try it out
tm <- seq(from = 1, to = 500) # 500 years 
Cinitial <- 30 # initial carbon 
...
result1 = getcarbon(Cinitial = 20, tm = seq(from = 1, to = 100), K = 100, r = 0.2, harv = 0.2, hfunc = dharvest)

harvestr = seq(from = 0.0, to = 0.4, by = 0.025)
tm = seq(from = 1, to = 200)

allres = harvestr %>%  map_dfc(~getcarbon (Cinitial = 20, tm = tm, K = 100, r = 0.2, hfunc = dharvest, harv = .x))

# rows are time, columns are carbon for each harvest scenario

# add names based on harvest
colnames(allres) = harvestr
allres = as.data.frame(allres)
allres$time = tm

allresl = allres %>%  pivot_longer(names_to = "harvestr", values_to = "carbon", -time)
# add time
...
ggplot(allresl, aes(time, carbon, col = harvestr)) +
  geom_line()
