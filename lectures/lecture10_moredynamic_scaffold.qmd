...
# run the ode solver, using r = 0.05 (as defined above)
...
```{r, all, echo=TRUE }
# create a dataframe (growth_result) to store results with time as first column, and make r a value
r = 0.05
P0 = 1

growth_result = data.frame(time = seq(from = 1, to = 100))
# add a column using analytical growth model (exppop applied over time using map_dbl)
growth_result$Panalytic = growth_result$time %>%  map_dbl(~exppop(P0 = P0, r = r, T = .x))
# add a column using discrete growth model (discrete_logistic_pop applied over time using map_dbl)

growth_result$Pdiscrete = growth_result$time %>%  map_dbl(~discrete_logistic_pop(P0 = P0, r = r, T = .x))
# add a column for ODE result - two steps
tmp = ode(y = P0, times = growth_result$time, dexppop, parms = c(r))
growth_result$Pode = tmp[,2] # just want pop, not the time 
# gather both model results using a key called "Ptype", value "P"
# gather both model results using a key called "Ptype", value "P"
tmp <- growth_result %>% gather(key = "Ptype", value = "P", -time)

grnice_longer <- pivot_longer(names_to = , cols = )
...
# using diffusion parameters 0.5 s/m2, and area 10 m2

# a list is returned with our 3 data frames for concentration (conc), qin and qout

# used filled contour to plot results

# or if you prefer this orientation (Distance on x axis by transposing results)
