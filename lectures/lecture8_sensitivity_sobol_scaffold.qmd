...
# first set call it X (each one generates new samples)
...
sens_Catm_Sobol = sobolSalt(model = NULL, X1, X2, nboot = 100) # WHY use a bootstrap?


# Take a look at the Sobol generated sensitivity object
str(sens_Catm_Sobol) # X1, X2, and X --> 
...
#(use pmap_dbl from purrr, so it returns an array of doubles)
res <- pmap_dbl(parms, Catm)

# look at lengh of tresults
length(res)
...
# look at results
names(sens_Catm_Sobol) # S & T (S = Main effect; T = total effect)
...
```{r, alternative}
sens_Catm_Sobol2 <- sobolSalt(model = NULL, X1, X2, nboot = 100, scheme = "B") # Scheme = B for second order indices 
