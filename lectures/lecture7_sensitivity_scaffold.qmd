...
f
# pnorm to determine the proability of runoff with these combinations 
...
# first run with best guess of infiltration capacity of 2 mm/hr


# try sampling across infiltration uncertainty
  # soil infiltration capacity mean 5 (mm/hr),  sd 2 normally distributed


# apply the erosion model using map and unlist
...
# which parameters are we going to vary - make a vector of all the names
...
parm_quant = randomLHS(nsample, )

# add column names to our quantile matrix
parm = as.data.frame(matrix(nrow = nrow(parm_quant),ncol = ncol(parm_quant))) 
  # column names of pnames
...
# first set up a data frame to hold the samples for each parameter a row for each sample, column for each parameter

# again add column names
# for each parameter pick samples based on lhs quantiles
# Lets use several examples normal distribution (with 10% standard deviation) and uniform with +- 10%
# in reality we should pick distribution from knowledge about uncertainty in parameters
...
parm[, "Tmincoeff2"] <- qnorm(parm_quant[, "Tmincoeff2"], mean = -0.07, sd = 0.07 / pvar)
parm[, "Pcoeff1"] <- qnorm(parm_quant[, "Pcoeff1"], mean = -0.07, sd = 0.004 / pvar)
...
# reshape data for easy plotting by gathering across yield type 

# use ggplot to create a boxplot for each yield type, across parameter uncertainty

# note that you don't see the ranges because of the scale (min yield anomoly much smaller than max) - here's a more informative way to graph - scales = "free"


# cumulative distribution is stat_ecdf in ggplot


# plot parameter sensitivity
# a bit tricky but nice way to make it easy to plot all parameters against all values

# first combine parameter sets with output
tmp <- cbind.data.frame(yieldsd, parm)

# gather across yield type

# gather across parameters (but not including yields)

# now plot using facets for each yield type, parameter combination
