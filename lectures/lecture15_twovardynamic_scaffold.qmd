...
-   differential equations used for prediction, mechanistic understanding, and decision/management support

-   we have focused on models with one dependent variable (e.g population size, or concentration of a pollutant)
...
-   pollutant concentrations in a diffusion-advection model where the two pollutants also interact with each other (or microbes populations with pollutants)
...
# set parameters
pars <- c(rprey = 0.5, alpha = 0.3, eff = 0.12, pmort = 0.2)
# run the model
...
# first plot against time - use color for pred vs prey
p1 <- ggplot(as.data.frame(res), aes(pred, prey)) + geom_point() +
  labs(y = "prey", x = "Pred")

p1
# plot interactions x-y (pred vs prey) - use color for time)
p2 <- ggplot(resl, aes(time, pop, color = animal)) + geom_line() +
  labs(y = "pop", x = "time")

p2
# plot side by side with ggarrange
```

```{r}
{r}
# add a carrying capacity (K) 

lotvmod_k <- function(t, pop, pars) {
  with(as.list(c(pars, pop)), {
    # population prey 
    dprey <- rprey * (1-prey/k) - alpha * prey * pred
    # population predator 
...
resk <- ode(func = lotvmodK, y = currpop, times = days, parms = pars)

# rearrange for plo

p2 <- ggplot(as.data.frame(resk), aes(time, pop, color = animal)) + geom_line() +
  labs(y = "pop", x = "time")
...
np <- 200
# then sample parameters for both sets of samples
...
## Assignment 7 {.scrollable}

[Assignment 7](%22../assignments/Assignment7_twovar_ODE.qmd%22)
