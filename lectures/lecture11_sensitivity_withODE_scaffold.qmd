...
X1 = X1 %>% map_df(pmax, 0.0) # for every value in the df, it will take the maximum 
X2 = X2 %>% map_df(pmax, 0.0)

# create our sobel object and get sets ofparameters for running the model, use sobolSalt
sens_P = sobolSalt(model = NULL, X1, X2, nboot = 3) # standard bootstrapping 
str(sens_P)
...
# lets add names
colnames(sens_P$X) = c("r", "K") # K = carrying capacity, r = initial pop
...
compute_pop_metrics <- function(result, thresh) {
  ...
  idx = which(result$P > thresh)[1] # [1] finding the first instance where the threshold is hit
  # account for sitation with threshold is never reached
  idx = ifelese(is.na(idx), length(result$P), idx)
...
simtimes = seq(from = 1, to = 500, by = 2) # every 2 years
...
metricpars = list(thresh = 10)
p_wrapper(r =0.01, K = 150, Pinitial = 3, simtimes = seq(from = 1, to = 10), 
          odefunc = dpopgrowth, metricfun = compute_pop_metrics, metricpars = metricpars)
...
allresult = as.data.frame(sens_P$X) %>%  pmap(p_wrapper, Pinitial = Pinitial, simtimes = simtimes, odefunc = dpopgrowth, metricfunc = compute_pop_metrics, metricpars = list(thresh = 100))

# extract out results from pmap into a data frame
allres = allresult %>%  map_dfr(`[`, c("maxpop", "threshyear"))
...
ggplot(allresp, aes(K, threshyear, col = r)) + 
  geom_point() +
  labs(x = "K", y = "Year that threshhold is reached", col = "growth rate")
