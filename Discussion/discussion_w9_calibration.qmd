...
execute: 
  echo: TRUE
theme: solarized
resources: ["../lectures/img/"]
css: ["slides.css"]
editor: visual
---

## Some insightful comments from guest lectures {.scrollable}
...
![](../lectures/img/modeling_pipeline_two_row.svg){width="100%"}
...
![Model Comparison](../lectures/img/hydrometric.png)
...
![Fuzzy Metric](../lectures/img/fuzzy_metric.png)
...
![Sagehen](../lectures/img/Sagehenpic.png)

## RHESSys

![RHESSys](../lectures/img/RHESSys.png)
...
# try another year
ggplot(subset(msagel, wy == 1985), aes(as.Date(date), str, col=run))+geom_line()+theme(legend.position = "none") +
  geom_line(data=subset(msage, wy == 1985), aes(as.Date(date), obs), linewidth=2, col="black", linetype=2)+labs(y="Streamflow", x="Date")

#try a month from a year
ggplot(subset(msagel, wy == 1970 & month == 6), aes(as.Date(date), str, col=run))+geom_line()+theme(legend.position = "none") +
  geom_line(data=subset(msage, wy == 1970 & month == 6), aes(as.Date(date), obs), linewidth=2, col="black", linetype=2)+labs(y="Streamflow", x="Date")
```
...
#alternatively
param_runs = msage %>% select(-date, -month, -day, -year, -wy, -obs )
res = map_dbl(.x=param_runs, .f=nse, o=msage$obs)
...
pres = ggplot(data=as.data.frame(res)) + geom_boxplot(aes(y=res))
res_thresh = summary(res)["3rd Qu."]
accept_res = res[which(res > res_thresh)]
pres + geom_boxplot(data=as.data.frame(accept_res), aes(y=accept_res), col="red")

summary(res)
quantile(res, probs = c(0,0.25,0.5,0.75,1))
...
```{r}
# Modify NSE - you want to determine the mean streatmflow, not what the original formula is calculating 

nse_modified <- function(m, o, dates) {
  #err <- m - o
  meanobs <- mean(o)
  #mse <- sum(err * err)
  return(meanobs)
...
#  comput for a single month 
nse_monthly <- function(m, o, dates, month) {
  idx <- as.integer(format(dates, "%m")) == month
  m <- m[idx]
  o <- o[idx]
...
res = msage %>% 
  select(-date, -month, -day, -year, -wy, -obs) %>% 
  map_dbl(nse_monthly, o=msage$obs, dates=msage$date, month=7)
...
![Equifinality](../lectures/img/dottyplots.png)
...
![Equifinality](../lectures/img/weights.png)
